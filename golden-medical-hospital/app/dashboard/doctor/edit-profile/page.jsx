...
export default function DoctorEditProfile() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        if (user.image) setPreview((p) => ({ ...p, image: user.image }));
        if (user.certificate)
          setPreview((p) => ({ ...p, certificateName: "uploaded" }));
      } catch (err) {
  ...
  const handleFileChange = (e) => {
    ...
    if (name === "imageFile") {
      ...
      setForm((s) => ({ ...s, certificateFile: file }));
      setPreview((p) => ({ ...p, certificateName: "selected" }));
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      let uploadedCert = form.certificatePath || "";

      if (form.imageFile) {
        uploadedImage = await uploadFile(form.imageFile);
      }
      if (form.certificateFile) {
        uploadedCert = await uploadFile(form.certificateFile);
      }
      ...
      const body = {
        ...
        certificate: uploadedCert || undefined,
      };
      ...
      if (result.image) setPreview((p) => ({ ...p, image: result.image }));
      ...
      setForm((f) => ({
        ...
        certificatePath: result.certificate ?? f.certificatePath,
        password: "",
        confirmPassword: "",
      }));
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(to_bottom,#CAF0F8,#ADE8F4,#90E0EF)] p-6 font-merriweather">
      <div className="max-w-3xl mx-auto bg-white/90 p-8 rounded-2xl shadow-lg border border-[#48CAE4]/30">
        <h1 className="text-2xl font-bold text-[#023E8A] mb-4">Edit Profile — Doctor</h1>
        ...
        <form onSubmit={handleSave} className="grid gap-4">
          ...
          <div className="p-4 rounded border border-[#48CAE4] bg-[#F0FBFF]">
            <label className="block text-sm text-[#0077B6] font-semibold mb-3">Profile image</label>
          ...
          <div className="p-4 rounded border border-[#48CAE4] bg-[#F0FBFF]">
            <label className="block text-sm text-[#0077B6] font-semibold mb-2">Certificate (PDF or ZIP)</label>
            ...
            <p className="mt-2 text-xs text-[#023E8A]">
              {preview.certificateName ? "Certificate selected" : "No certificate uploaded"}
            </p>
          </div>

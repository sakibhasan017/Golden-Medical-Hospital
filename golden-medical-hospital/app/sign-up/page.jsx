...
const SignUpPage=()=> {
  ...
  const validate = (data, type) => {
    ...
    ['name', 'email', 'phone', 'age', 'password', 'confirmPassword'].forEach((f) => {
      if (!String(data[f] || '').trim()) err[f] = `${f} is required`;
    });
    ...
    if (type === 'patient' && !String(data.bloodGroup || '').trim())
  ...
  const handleSubmit = async (e, type) => {
    ...
    try {
      ...
      // append all fields explicitly to avoid unexpected serialization issues
      Object.entries(data).forEach(([k, v]) => {
        if (v !== null && v !== undefined && v !== '') {
          // File objects are appended as-is
          formData.append(k, v);
        }
      });
      ...
      // try to auto sign-in using credentials after successful registration
      // ensure backend stored the user and password was hashed — signIn will verify.
      const email = (data.email || '').trim();
      const password = data.password; // user-entered plain password

      // attempt sign-in via credentials provider (no redirect)
      ...
      if (signInResult?.error) {
        // signIn failed — show message but still show success note for registration
        setMessage({ type: 'success', text: json.message || 'Registered. Please login manually.' });
        setLoading(false);
        return;
      }

      // signIn success -> redirect according to role
      if (type === 'patient') {
        router.replace('/dashboard/patient');
      } else {
        // doctor registered -> show waiting page until admin approves
        router.replace('/doctor-waiting');
      }
    } catch (err) {
      console.error(err);
